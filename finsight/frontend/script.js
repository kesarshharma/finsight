...
// Educational info for each metric
const infoData = {
  price: {
...
async function fetchData() {
  ...
  try {
    // Basic analytics (price, SMA, volatility)
    ...
    // Enhanced – now always returns JSON, even on rate‑limit (fallback data)
    const enhancedRes = await fetch(`/enhanced/${symbol}`);
    const enhanced = await enhancedRes.json();   // no throw, always valid
    ...
    // Enhanced cards – gracefully show "—" for missing values
